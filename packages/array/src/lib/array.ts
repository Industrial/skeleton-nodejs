 ...
 */
export const sliceE = (start: number | undefined, end: number | undefined) => (as: Array<number>) =>
...
export const allIndexesOf = <T>(item: T, array: Array<T>): Array<number> =>
  pipe(
    array,
    A.filterMap((value, index) => (Equal.equals(value, item) ? Option.some(index) : Option.none())),
  )
 ...
 */
export const average = (values: Array<number>): Option.Option<number> => pipe(N.sumAll(values), N.divide(values.length))
